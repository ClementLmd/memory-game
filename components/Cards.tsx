...
export default function Cards() {
  ...
  const resetGame = () => {
    ...
    if (selectedTheme) {
      setCards(prepareCards(selectedTheme));
    }
  };
  ...
  useEffect(() => {
    if (flippedCards.length === 2) {
      // Move this outside the effect to avoid the loop
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-6xl mx-auto">
      <div className="mb-4 flex flex-col sm:flex-row justify-between w-full px-4">
        <div>
          <h2 className="font-bold text-xl">{selectedTheme.name}</h2>
          <p>Moves: {moves}</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-4 w-full px-2">
        {cards.map((card) => (
          <Card
            key={card.id}
            id={card.id}
